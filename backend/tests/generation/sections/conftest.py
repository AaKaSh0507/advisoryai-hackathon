...
def mock_llm_client() -> MockLLMClient:
    return MockLLMClient(default_response="This is generated content for the advisory section.")
...
async def db_output_repository(db_session) -> SectionOutputRepository:
    return SectionOutputRepository(db_session)


@pytest_asyncio.fixture
async def db_input_repository(db_session) -> GenerationInputRepository:
    return GenerationInputRepository(db_session)


@pytest_asyncio.fixture
async def db_generation_service(
    db_output_repository: SectionOutputRepository,
    db_input_repository: GenerationInputRepository,
    mock_llm_client: MockLLMClient,
) -> SectionGenerationService:
    return SectionGenerationService(
        output_repo=db_output_repository,
        input_repo=db_input_repository,
        llm_client=mock_llm_client,
    )
